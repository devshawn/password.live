import { PasswordAPI } from "../../application/password/password.api"

describe("PasswordAPI", () => {
    describe("PasswordAPI.fetchPassword", () => {
        describe("default behavior", () => {
            const { password } = PasswordAPI.fetchPassword()
            ...
            it("it returns unique passwords", () => {
                expect(password).not.toBe(PasswordAPI.fetchPassword())
            })
        })

        describe("with body.uppercase = true", () => {
            const { password } = PasswordAPI.fetchPassword({ length: 100, uppercase: true })
        ...
        describe("with body.numbers = true", () => {
            const { password } = PasswordAPI.fetchPassword({ length: 100, numbers: true })
        ...
        describe("with body.symbols = true", () => {
            const { password } = PasswordAPI.fetchPassword({ length: 100, symbols: true })
        ...
        describe("with body.length = n between 1 and 100", () => {
            ...
            const { password } = PasswordAPI.fetchPassword({ length })
